...
function PaidReportContent() {
  ...
  const rows = useMemo(
    () => [...(projectFinancials.data ?? [])].sort((a, b) => b.paidAmount - a.paidAmount),
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">QuickBooks Paid Report</h1>
          <p className="text-sm text-muted-foreground">Projects ordered by paid amount</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Paid Details</CardTitle>
        </CardHeader>
        <CardContent>
          {projectFinancials.isLoading ? (
            <p className="text-sm text-muted-foreground">Loading paid financials...</p>
          ) : projectFinancials.error ? (
            <p className="text-sm text-destructive">Could not load paid financials.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
...
export default function QuickbooksPaidReportPage() {
  return (
