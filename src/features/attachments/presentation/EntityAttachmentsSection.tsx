...
import {
  getEntityPresignedUploadUrl,
  getEntityPresignedDownloadUrl,
  getEntityPresignedPreviewUrl,
  type AttachmentEntityKind,
} from "@/features/attachments/actions/s3Actions";
import { FilePreviewModal } from "@/features/leads/presentation/pages/sections/FilePreviewModal";
...
function AttachmentThumbnail({
  ...
  useEffect(() => {
    ...
    getEntityPresignedPreviewUrl(keyName)
      .then((nextUrl) => {
...
export function EntityAttachmentsSection({
  ...
  const uploadFiles = useCallback(
    ...
    [entityKind, entityId, attachments, onAttachmentsChange],
  );
  ...
  const handleDownload = async (key: string) => {
    ...
    try {
      const url = await getEntityPresignedDownloadUrl(key);
  ...
  const handlePreview = async (key: string) => {
    try {
      const url = await getEntityPresignedPreviewUrl(key);
