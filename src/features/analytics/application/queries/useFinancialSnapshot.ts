...
export function useFinancialSnapshot(leadType?: LeadType) {
  const ctx = useAnalyticsApp();

  return useQuery({
    ...analyticsQueryDefaults,
    queryKey: analyticsKeys.financialSnapshot(leadType),
    queryFn: () => ctx.repos.analytics.getFinancialSnapshot({ leadType }),
  });
}
