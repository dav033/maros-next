...
type DiContextValue = {
  ...
  quickbooksApp: QuickbooksAppContext;
...
export function DiProvider({ children }: Props) {
  const value = useMemo(() => ({
    ...
    quickbooksApp: createQuickbooksAppContext(),
...
export function useQuickbooksApp(): QuickbooksAppContext {
  const context = useContext(DiContext);
  if (!context) {
    throw new Error("useQuickbooksApp must be used within DiProvider");
  }
  return context.quickbooksApp;
}

export function useNotesApp(): NotesAppContext {
  const context = useContext(DiContext);
  if (!context) {
    throw new Error("useNotesApp must be used within DiProvider");
  }
  return context.notesApp;
}

export function useUsersApp(): UsersAppContext {
  const context = useContext(DiContext);
  if (!context) {
    throw new Error("useUsersApp must be used within DiProvider");
  }
  return context.usersApp;
}

export function useTasksApp(): TasksAppContext {
  const context = useContext(DiContext);
  if (!context) {
